...
## Criterios extra (meta-repo pr-loop / bash)

- **Bash**: scripts con `set -euo pipefail`; sin lógica de negocio pesada en wrappers de `scripts/`.
- **Compatibilidad**: flags y variables de entorno documentados en README siguen funcionando.
- **Zonas protegidas**: el pipeline no debe mergear automáticamente ni editar `progress/` en reviews.
- **Dry-run**: cambios en orquestador o estado deben respetar `PR_LOOP_DRY_RUN` / `--dry-run`.
